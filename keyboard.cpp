...
commandport(0x64)
{
    ...
    while(commandport.Read() & 0x1)
        dataport.Read();
    commandport.Write(0xAE); // activar interrupts
    commandport.Write(0x20); // devuelve el estado actual
    uint8_t status = (dataport.Read() | 1) & ~0x10;
    commandport.Write(0x60); // cambiar estado
    dataport.Write(status);

    dataport.Write(0xF4); // activa el teclado
    //
